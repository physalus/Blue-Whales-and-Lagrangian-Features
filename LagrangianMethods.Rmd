...
author: "James A. Fahlbusch, Max F. Czapanskiy, John Calambokidis, David E. Cade, Briana Abrahms, Elliott L. Hazen, Jeremy A. Goldbogen"
...
# Comparison
if (file.exists("./Models/feed_glmmPQL_CompareCRW_CAR1_Hrs.rds")) {
  feed_glmmPQL_CompareCRW_CAR1_Hrs <-
    readRDS("./Models/feed_glmmPQL_CompareCRW_CAR1_Hrs.rds")
} else { 
  cat(paste0("Could not find ./Models/feed_glmmPQL_CompareCRW_CAR1_Hrs.rds, will need to recreate below."))
}
...
```{r DeploymentOverView, echo=TRUE, message=FALSE, warning=FALSE}
...
* TRACE Parameters
  ...
  + Number of Initial Times: Number of hours in the deployment + 48 
    - this ensures the data will envelop the deployment by $\pm$1 day
...
![Example animation of Blue Whale and FTLE](./rmdfiles/Fahlbusch_BlueWhaleFTLE.gif)
...
if(!exists("feed_glmmPQL_CompareCRW_CAR1_Hrs")){
  feed_glmmPQL_CompareCRW_CAR1_Hrs <- glmmPQL(Foraging ~ ftle48 + method + ftle48*method, 
                                              random = ~ 1 | depid,
                                              correlation=corCAR1(form=~timeHrs|depid),
                                              family = binomial,
...
```{r suppTable1, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r hmm2StateGapCheck, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r plotGLMMVSHMM,cache=TRUE, echo=TRUE, fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
...
```{r individualFR,cache=TRUE, echo=TRUE, fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
p1 <- ggplot() +
...
```{r hmmDetermineFRPercentiles,cache=TRUE, echo=TRUE, fig.height=8, fig.width=12, message=FALSE, warning=FALSE}
...
#### Prepare the data: 
```{r hmmDataPrep4State,cache=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
ftleGrid <- seq(-1.25,1.5, length = 1000)
# Design matrix for prediction of stationary probabilities
covs <- cbind("intercept" = 1,
              "ftle" = ftleGrid)
...
pHMM4 <- ggplot()+
  ...
  geom_ribbon(data=ciDF %>% filter(State=="Heavy Feeding"),
              aes(x=ftle,ymin=ciLwr,ymax=ciUpr,color=NULL,
                  group=State,fill=State), alpha = .15) +
  geom_line(data=stat3_DF %>% filter(State=="Not Feeding"),aes(x=FTLE,y=Probability,group=State,
  ...
  geom_line(data=stat3_DF %>% filter(State=="Moderate Feeding"),
            aes(x=FTLE,y=Probability,group=State,color=State,
                linetype=State),size=2)+
  geom_line(data=stat3_DF %>% filter(State=="Heavy Feeding"),aes(x=FTLE,y=Probability,group=State,
                                                                color=State,linetype=State),size=2)+
  scale_color_manual(name=NULL,
...
```{r table2,cache=TRUE, echo=TRUE, message=FALSE, warning=FALSE}
# Encountered FTLE
